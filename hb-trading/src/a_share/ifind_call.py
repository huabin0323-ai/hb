...
_CFG = Path(__file__).parent / "mcp_config.json"
if not _CFG.exists():
    _CFG = Path(__file__).parent.parent.parent / "mcp_config.json"
...
# Thread-safe: protect session init + all calls with single lock
# (iFinD MCP shares one session per server type — concurrent calls corrupt it)
...
def _init(t):
    """Session initialization — caller must hold _lock"""
    if t in _initialized and t in _sessions:
        return
        ...
        if not session_id:
            raise RuntimeError(f"initialize succeeded but no Mcp-Session-Id: {data}")
...
def call(server_type, tool_name, params, timeout=60):
    """Thread-safe MCP tool call — serialized via global lock"""
    ...
    with _lock:
        ...
        if isinstance(data, dict) and "error" in data:
            err_msg = data["error"]
            # If session expired, retry once with re-init
            if "session" in str(err_msg).lower() or "Mcp-Session-Id" in str(err_msg):
                _sessions.pop(server_type, None)
                _initialized.discard(server_type)
                _init(server_type)
                resp, data = _post(server_type, payload, timeout=timeout)
                if isinstance(data, dict) and "error" in data:
                    return {"ok": False, "status_code": resp.status_code, "error": data["error"], "raw": data}

            return {"ok": False, "status_code": resp.status_code, "error": err_msg, "raw": data}
...
if __name__ == "__main__":
    print("iFinD MCP call module ready — use call(server_type, tool_name, params)")
